...
module OpenProject::GitHosting
  class Admin
    ...
    def initialize(object_id, action, options)
      @gitolite_admin_dir                = Config.gitolite_admin_dir
      @gitolite_config_file              = ConfigRedmine.get_setting(:gitolite_config_file)
      @gitolite_default_config_file      = Config::GITOLITE_DEFAULT_CONFIG_FILE
      ...
      @gitolite_identifier_prefix        = ConfigRedmine.get_setting(:gitolite_identifier_prefix)

      @delete_git_repositories   = ConfigRedmine.get_setting(:delete_git_repositories, true)
      @gitolite_server_port      = ConfigRedmine.get_setting(:gitolite_server_port)
      @gitolite_admin_url        = Config.gitolite_admin_url
      @gitolite_admin_ssh_script_path = Config.gitolite_admin_ssh_script_path
      @lock_file_path = File.join(Config.get_temp_dir_path, 'openproject_git_hosting_lock')
      @gitolite_debug = ConfigRedmine.get_setting(:gitolite_log_level) == 'debug' ? true : false
      @gitolite_timeout = ConfigRedmine.get_setting(:gitolite_timeout).to_i
      @gitolite_author  = Config.gitolite_commit_author
    ...
    def gitolite_admin_repo_clone
      ...
      if !Gitolite::GitoliteAdmin.is_gitolite_admin_repo?(@gitolite_admin_dir)
        ...
        begin
          GitHosting.execute_command(:local_cmd, "rm -rf '#{@gitolite_admin_dir}'")
          GitHosting.execute_command(:local_cmd, "env GIT_SSH=#{@gitolite_admin_ssh_script_path} git clone ssh://#{@gitolite_admin_url} #{@gitolite_admin_dir}")
          GitHosting.execute_command(:local_cmd, "chmod 700 '#{@gitolite_admin_dir}'")
      ...
      if @gitolite_config_file != @gitolite_default_config_file
        if !File.exists?(@gitolite_config_file_path)
          begin
            GitHosting.execute_command(:local_cmd, "touch '#{@gitolite_config_file_path}'")
          rescue GitHostingException => e
        ...
        begin
          include_present = GitHosting.execute_command(:local_cmd, "grep '#{@gitolite_config_file}' #{@gitolite_default_config_file_path} | wc -l")
        rescue GitHostingException => e
          logger.error { e.message }
