    ...
    module RepositoryPatch
      ...
      def self.included(base)
        base.class_eval do
          ...
          extend ClassMethods
          class << self
            alias_method_chain :factory, :revisions_git
          end
        end
      end
      ...
      module ClassMethods
        ...
        def factory_with_revisions_git(klass_name, *args)
          new_repo = factory_without_revisions_git(klass_name, *args)
          if new_repo.is_a?(Repository::Git)
            if new_repo.extra.nil?
              # Note that this autoinitializes default values and hook key
              OpenProject::Revisions::Git::GitoliteWrapper.logger.error { "Automatic initialization of RepositoryGitExtra failed." }
            end
          end
          return new_repo
        end
