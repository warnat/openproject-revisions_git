class AddSettingsToPlugin < ActiveRecord::Migration
  def self.up
    begin
      ...
      if (Setting.plugin_openproject_git_hosting != valuehash)
        ...
        say "Added openproject_git_hosting settings: 'gitRecycleBasePath', 'getRecycleExpireTime', 'getLockWaitTime', 'httpServer'"
      end
    rescue => e
      puts e.message
    end
  ...
  def self.down
    begin
      ...
      if (Setting.plugin_openproject_git_hosting != valuehash)
        Setting.plugin_openproject_git_hosting = valuehash
        say "Removed openproject_git_hosting settings: 'gitRecycleBasePath', 'getRecycleExpireTime', 'getLockWaitTime', 'httpServer'"
      end
      Setting.plugin_openproject_git_hosting = valuehash
    rescue => e
      puts e.message
